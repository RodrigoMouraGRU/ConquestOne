...
namespace ProtectosScafold.Controllers
{
    public class AdministradoraController : BaseController
    {
        ...
        {
            ViewBag.AdministradoraId = id;
            return PartialView("_TelefonesList", _administradoraApplicationService.AdministradoraObterPorId(id).Telefones);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AdicionarTelefone(AdministradoraTelefoneViewModel administradoraTelefoneViewModel)
        {
            if (ModelState.IsValid)
            {
                _administradoraApplicationService.AdministradoraTelefoneAdicionar(administradoraTelefoneViewModel);
                string url = Url.Action("ListarTelefones", "Administradora", new { id = administradoraTelefoneViewModel.AdministradoraId });
                return Json(new { success = true, url = url });
            }
            return PartialView("_AdicionarTelefone", administradoraTelefoneViewModel);
        }
        public ActionResult AtualizarTelefone(Guid id)
        {
            return PartialView("_AtualizarTelefone", _administradoraApplicationService.AdministradoraTelefoneObterPorId(id));
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AtualizarTelefone(AdministradoraTelefoneViewModel administradoraTelefoneViewModel)
        {
            if (ModelState.IsValid)
            {
                _administradoraApplicationService.AdministradoraTelefoneAtualizar(administradoraTelefoneViewModel);

                string url = Url.Action("ListarTelefones", "Administradora", new { id = administradoraTelefoneViewModel.AdministradoraId });
                return Json(new { success = true, url = url });
            }

            return PartialView("_AtualizarTelefone", administradoraTelefoneViewModel);
        }
        public ActionResult DeletarTelefone(Guid? id)
        {
            ...
            var telefoneViewModel = _administradoraApplicationService.AdministradoraTelefoneObterPorId(id.Value);
        ...
        {
            var administradoraId = _administradoraApplicationService.AdministradoraTelefoneObterPorId(id).AdministradoraId;
            _administradoraApplicationService.DeleteAdministradoraTelefone(id);
            string url = Url.Action("ListarTelefones", "Administradora", new { id = administradoraId });
            return Json(new { success = true, url = url });
        }
